...
function Scrollbar(_ref) {
  ...
  var horItemDom = (0, _react.useRef)(null); //储存滚动条滚动信息
  ...
  var mouseWheelSetTime = (0, _react.useRef)(null); //滚动条显示变量
  ...
  var handlemouseWheel = (0, _react.useCallback)(function (e) {
    ...
    handleScrollFrame();
    return; //onwheel 事件写法，废弃
    // let deltaY = 125; //e.deltaY

    var deltaY = 150; //e.deltaY
    ...
    var t_scroll = barState.tableClientHeight - verItemDomHeight;
    var v_top = parseFloat(verItemDom.current.style && verItemDom.current.style.top || 0);
    var dis = v_top + deltaY / barState.tableScrollHeight * barState.tableClientHeight * 1.9; //减少没必要的计算
    ...
    mouseWheelSetTime.current = setTimeout(function () {
      verItemDom.current.style.top = dis + 'px';
      var scrollDis = dis / (barState.tableClientHeight - verItemDomHeight) * (barState.tableScrollHeight - barState.tableClientHeight);
